defmodule Sawver.SkillBook do
  ...
  def changeset(%SkillBook{} = skill_book, attrs) do
    skill_book
    |> cast(attrs, [:tree, :display_name, :name, :type, :description, :prereqs, :cost, :cooldown])
  ...
  def insert_initial_skill_values() do
    ...
    initial_books = [
      ...
      %{tree: "gatherer", display_name: "Gather Cloth", name: "gatherCloth", type: "passive", description: "You think you can fashion cloth from the wood fibers if you chop the trees juuuust right.", prereqs: [], cost: 1, cooldown: nil},
      %{tree: "gatherer", display_name: "Gather Rope", name: "gatherRope", type: "passive", description: "Logic dictates that chopping even more carefully should allow you to produce rope.", prereqs: ["gatherCloth"], cost: 1, cooldown: nil},
      %{tree: "gatherer", display_name: "Gather Magic", name: "gatherMagic", type: "passive", description: "Somehow chop trees in a way that allows you pull magic out of them. I don't know. You come up with some flavor text.", prereqs: ["gatherRope"], cost: 1, cooldown: nil},
      %{tree: "gatherer", display_name: "Gather Gem", name: "gatherGem", type: "passive", description: "Has someone been hiding gems in these trees this whole time?!", prereqs: ["gatherMagic"], cost: 1, cooldown: nil},
      ...
      %{tree: "gatherer", display_name: "Chop-Dash", name: "chopDash1", type: "active", description: "You know, it might be faster to just not stop to swing the axe.", prereqs: ["gatherGem", "chopFast3"], cost: 1, cooldown: 25.0},
      %{tree: "gatherer", display_name: "Adv. Chop-Dash", name: "chopDash2", type: "active", description: "Yes, totally! Swinging is for chumps! Let's just get better at this chop-dash thing!", prereqs: ["chopDash1"], cost: 1, cooldown: 12.0},
      ...
      %{tree: "leader", display_name: "Boost Walk", name: "buffWalk", type: "passive_aura", description: "Inspire others to walk more briskly.", prereqs: ["emote"], cost: 1, cooldown: nil},
      %{tree: "leader", display_name: "Boost Chop", name: "buffChop", type: "passive_aura", description: "Your instructions on how to chop properly increases nearby lumberjack gathering ability and totally doesn't make them think you act like a know it all.", prereqs: ["buffWalk", "emoteMore"], cost: 1, cooldown: nil},
      %{tree: "leader", display_name: "Boost Build", name: "buffBuild", type: "passive_aura", description: "Lower the cost for nearby lumberjacks to build things by becoming the person in the group effort that always says they are 'supervising'.", prereqs: ["trackBuilding", "emoteMore"], cost: 1, cooldown: nil}
    ]
    |> Enum.map(fn(row) ->
      row
  ...
  def get_skill_list_for_player() do
    ...
    |> Enum.map(fn(sb) -> Map.take(sb, [:tree, :display_name, :name, :description, :prereqs, :cost, :cooldown]) end)
  end
end
